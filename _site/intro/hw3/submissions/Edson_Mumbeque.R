...
## 2. Get weather data for Maputo from 2010 to 2016 
...
## 6. Create an object called y. This will be x, 
##then grouped by water, then summarise by creating a new variable called number_of_days 
...
## 10.mNow, you're going to overwrite weather with itself (weather <- weather), but then mutate a new variable called too_hot_to_sleep. This will be "yes" if the temp_min is greater than 28, and otherwise it will be "no". 
weather <- 
  weather %>%
...
## 13. Overwrite weather with weather again, but this time using mutate to create three variables: year, month and day. I
weather <- 
  weather %>%
